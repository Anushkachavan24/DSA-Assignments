...
void heapify(heap *h){
    ...
    while(i >= 0){
        ...
        if(h->arr[p] < h->arr[i]){
        //Swap the current element with its parent if parent is smaller than current element
...
void insertheap(heap *h, int a){
    ...
    //Increment the top and insert element to it
         ...
         //if parent is less than inserted then call heapify
              int p = (h->top -1)/2;
...
int deleteheap(heap *h){
    ...
    while(i<h->top){
         ...
         if(il <= h->top){
              //if ir is not present then max = il
              if(ir > h->top){
                  max = il;         
              }
              //if ir is present then compare il and ir and set max
              else if(ir <= h->top){
              ...
              //if max is larger than i then swap
              if(h->arr[max] > h->arr[i]){
